...
router.get('/api',function(req,res){
    Artist.getArtistsFullInfo(function(err,callback){
      if(err)
      {
        res.json('Error');
      }
...
router.get('/add',function(req,res){
    res.render("Artist/Add/ArtistAdd")
});
router.post('/add',function(req,res){
  var storage = multer.diskStorage({
    destination: function (req, file, cb) {
      //cb(null, 'artistsMedia/drake/songs')
      var dir = 'artistsMedia/' + req.body.name+ '/images';
      mkdirp(dir,err => cb(err,dir))
    },
    filename: function (req, file, cb) {
      console.log(file);
      cb(null, req.body.name +'_'+ file.fieldname +'_'+ file.originalname) //Appending extension
    },
