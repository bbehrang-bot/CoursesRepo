...
cJSON * teacher_mJSON(const teacher * teachers) {
	...
	cJSON_AddItemToObject(teachersJSON, "student", cJSON_CreateString(teachers->firstName));
	cJSON_AddItemToObject(teachersJSON, "group", cJSON_CreateString(teachers->lastName));
	cJSON_AddItemToObject(teachersJSON, "var", cJSON_CreateNumber(teachers->id));
