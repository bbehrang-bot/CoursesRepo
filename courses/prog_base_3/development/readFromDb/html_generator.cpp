...
std::wstring html_generator::table_data_html(const std::vector<std::vector<std::pair<std::string, std::string>>>& table_data, const std::string table_name)
{
	...
	html_out += U("<a href=\"http://localhost:8090/Admin\">Back to managment panel</a>\n");
	html_out += U("    </body>\n");
	html_out += U("  </html>\n");
	return html_out;
}
...
std::wstring html_generator::input_form_html_edit(const std::vector<std::vector<std::pair<std::string, std::string>>>& table_data, const std::string table_name)
{
	...
	for (std::vector<std::pair<std::string, std::string>> row_data : table_data) {
		...
		if (table_data.size() > 0) {
			...
			html_out += U("    <tr>\n");
			html_out += U("      <td><input type='submit' value='SAVE EDIT'>");
			html_out += U("				<script>");
			html_out += U("							function deleteFromDB(){var xhttp = new XMLHttpRequest();xhttp.open('DELETE','") + std::wstring(service_url.begin(), service_url.end()) + U("/Admin/") + std::wstring(table_name.begin(), table_name.end()) + U("/delete/") + std::wstring(row_id.begin(), row_id.end()) + U("',true);xhttp.send()}");
			html_out += U("				</script>");
			html_out += U("<a onclick=\"deleteFromDB()\">remove</a>");
			html_out += U("</td>\n");
			html_out += U("    </tr>\n");
		}
		html_out += U("  </table>\n");
