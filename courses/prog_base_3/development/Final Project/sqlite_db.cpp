...
int sqlite_db::delete_order(std::string row_id)
{
	int rc;
	std::string sql_query;
	...
	sql_query = "DELETE FROM OrderTable WHERE id = " + row_id + ";";
	rc = sqlite3_exec(db, sql_query.c_str(), sqlite_db::callback, 0, &zErrMsg);
	if (rc != SQLITE_OK) {
		printf("SQL error: %s\n", zErrMsg);
		sqlite3_free(zErrMsg);
	...
	sql_query = "DELETE FROM OrderProductBridge WHERE OOrderId = " + row_id + ";";
	rc = sqlite3_exec(db, sql_query.c_str(), sqlite_db::callback, 0, &zErrMsg);
	if (rc != SQLITE_OK) {
		printf("SQL error: %s\n", zErrMsg);
		sqlite3_free(zErrMsg);
...
int sqlite_db::db_getOrders(std::vector<std::vector<std::pair<std::string, std::string>>>& table_rows,int id)
{
	...
	sql_query = "SELECT OProductId,OCount FROM OrderProductBridge Where OorderId =" + std::to_string(id) + ";";
	rc = sqlite3_exec(db, sql_query.c_str(), sqlite_db::callback, 0, &zErrMsg);
	if (rc != SQLITE_OK) {
		printf("SQL error: %s\n", zErrMsg);
		sqlite3_free(zErrMsg);
...
int sqlite_db::db_placeOrder(Product product, int count,int orderTableId)
{
	int rc = -1;
	sqlite3_stmt * stmt = NULL;
	char * sqlQuery = "INSERT INTO OrderProductBridge ('OProductId','OOrderId','OCount') VALUES (?,?,?);";
