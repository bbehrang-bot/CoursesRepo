...
void main_page_responser(http_request request) {
	...
	db.get_table_data("Slider", table_data, 50, 0);
	...
	response.set_body(html_gen.html_homePage(html_body,company,table_data));
...
void table_page_responser(http_request request) {
	...
	else if (path.at(1) == U("Messages"))
	{
		table_name = "Messages";
	}
	else if (path.at(1) == U("Slider"))
	{
		table_name = "Slider";
	}
	if (path.size() >= 3) {
	...
	{
		response.set_body(html_gen.html_companyInfo(table_data, table_name));
	}
	else if (path.at(1) == U("Messages"))
	{
		response.set_body(html_gen.html_MessagesAdmin(table_data, table_name));
	}
	else
	...
	{
		std::wstring html_out = html_gen.html_Alert("Wrong username/password :(");
		std::wstring out;
		out = html_gen.html_render_body(html_out, company);
		response.set_body(out);
		request.reply(response);
...
void order_page_responser(http_request request) {
	...
	if (totalPrice == 0)
	{
		out = html_gen.html_Alert("Something went wrong :(");
		 ...
		 rc = db.db_placeOrderTable(postData,totalPrice);
	...
	if(orderTableId != -1 && orderTableId != 0 && totalPrice != 0 )
